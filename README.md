...
以中文学习者为核心的日语主课程产品，基于 `React + Vite + TypeScript`。课程负责决定下一步学什么、验证是否真正回忆出来，并在接近遗忘时安排复习；歌曲和视频作为课程知识的真实语境。

## 功能

- 从入门到 N1 准备阶段的连续课程地图与水平定位
- 每课中文讲解、日语朗读、主动回忆检测和逐题反馈
- 基于答题正确率、反应时间与延迟回忆的知识掌握度
- 动态复习队列、课程进度与本地/云端恢复
- 短视频沉浸式学习流
...
歌曲导入完成后，Codex 学习信息由独立常驻 Worker 异步生成，不再要求访问者运行本地服务。队列、Worker、Codex skill、登录和 Docker 部署说明见 [云端 Codex 学习信息 Worker](./docs/song-analysis-worker.md)。
