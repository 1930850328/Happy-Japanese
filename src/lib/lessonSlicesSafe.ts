...
function buildRoughSliceLessons(clip: ImportedClip) {
  const segments = clip.segments
    .filter((segment) => segment.endMs > segment.startMs)
    .slice(0, MAX_SLICES_PER_CLIP)
  ...
  return segments.map((segment, index) => {
    ...
    const titleBase = shortenTitle(clip.sourceAnimeTitle ?? clip.title)

    return {
      id: `${clip.id}::rough-slice-${index + 1}`,
      originClipId: clip.id,
      sourceType: clip.sourceType,
      sourceIdOrBlobKey: clip.sourceIdOrBlobKey,
      sourceFileName: clip.sourceFileName,
      sourceUrl: clip.sourceUrl,
      sourceProvider: `${clip.sourceProvider} / 无字幕粗切`,
      title: `${titleBase} - 粗切片 ${index + 1}`,
      cover: clip.cover,
      theme: clip.theme,
      difficulty: clip.difficulty,
      durationMs,
      ...
      segments: normalizeSegments([segment], segment.startMs),
      knowledgePoints: [],
      tags: [...new Set([...clip.tags.filter((tag) => tag !== '待生成字幕'), '时间粗切'])],
      description: '字幕模型暂时不可用，这条先按时间粗切，适合先做听写、跟读，之后也可以补导入字幕。',
      creditLine: clip.creditLine,
      sliceLabel: `${Math.max(4, Math.round(durationMs / 1000))} 秒粗切片`,
      feedPriority: 118 - index,
...
function hasMeaningfulStudyContent(clip: ImportedClip) {
  return clip.knowledgePoints.some(
    (point) =>
      point.id !== 'local-subtitle-tip' && (point.kind === 'grammar' || point.kind === 'word'),
  )
}

function shouldBuildRoughSlices(clip: ImportedClip) {
  return clip.importMode === 'raw' && clip.subtitleSource === 'auto' && clip.tags.includes('字幕兜底')
}
...
export function buildLessonsFromImportedClip(clip: ImportedClip) {
  ...
  if (selected.length === 0) {
    if (shouldBuildRoughSlices(clip)) {
      return buildRoughSliceLessons(clip)
    }
